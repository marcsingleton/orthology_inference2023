...
for label in [path for path in os.listdir('../iqtree_GTR2/out/') if os.path.isdir(f'../iqtree_GTR2/out/{path}')]:
    trees = []
    for file in [path for path in os.listdir(f'../iqtree_GTR2/out/{label}/') if path.endswith('.treefile')]:
        tree = skbio.read(f'../iqtree_GTR2/out/{label}/{file}', 'newick', skbio.TreeNode)
...
"""
DEPENDENCIES
../iqtree_GTR2/iqtree_GTR2.sh
    ../iqtree_GTR2/out/*/meta_*.treefile
"""