...
rows = []
aligners = [name for name in os.listdir('../m_coffee_align/out/') if os.path.isdir('../m_coffee_align/out/' + name)]
for aligner in aligners:
    for ref in ['11', '12', '20', '30', '40', '50']:
        file_ids = [name[:-4] for name in os.listdir(f'../m_coffee_align/out/{aligner}/RV{ref}/') if name.endswith('.mfa')]
        for file_id in file_ids:
            print(aligner, ref, file_id)
            ref_seq = read_msf(f'../../../data/bb3_release/RV{ref}/{file_id}.msf')
            test_seq = read_fasta(f'../m_coffee_align/out/{aligner}/RV{ref}/{file_id}.mfa')
            seqids = list(ref_seq)
...
DEPENDENCIES
../../../data/bb3_release/
../m_coffee_align/m_coffee_align.py
    ../m_coffee_align/out/*
"""