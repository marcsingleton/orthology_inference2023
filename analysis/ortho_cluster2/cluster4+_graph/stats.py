...
for k in os.listdir('out/'):
    # Load OGs
    component2OGs = {}
    with open(f'out/{k}/clusters.tsv') as file:
        field_names = file.readline().rstrip('\n').split('\t')
        for line in file:
            fields = {key: value for key, value in zip(field_names, line.rstrip('\n').split('\t'))}
    ...
    plt.legend()
    plt.savefig(f'out/{k}/bar_connectnum-OGnum_type_dist1-1.png')
    plt.xlim((-1, 17))  # Adjust axis to truncate outliers
    plt.savefig(f'out/{k}/bar_connectnum-OGnum_type_dist1-2.png')
    plt.close()
    ...
    plt.ylabel('Number of connected components')
    plt.legend()
    plt.savefig(f'out/{k}/bar_connectnum-OGnum_type_dist2-1.png')
    plt.xlim((-1, 17))  # Adjust axis to truncate outliers
    plt.savefig(f'out/{k}/bar_connectnum-OGnum_type_dist2-2.png')
    plt.close()
    ...
    plt.savefig(f'out/{k}/pie_component_type.png')
    plt.close()
