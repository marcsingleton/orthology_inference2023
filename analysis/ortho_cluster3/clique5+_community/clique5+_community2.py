...
def save_results(OGs, CCtypes, k):
    ...
    if not os.path.exists(f'out/ggraph2/{k}clique/'):
        os.makedirs(f'out/ggraph2/{k}clique/')  # Recursive folder creation

    # Write OGs to file
    j = 0
    with open(f'out/ggraph2/{k}clique/gclusters.txt', 'w') as file:
    ...
    plt.legend()
    plt.savefig(f'out/ggraph2/{k}clique/connectnum-OGnum_type_dist1-1.png')
    plt.xlim((-1, 17))  # Adjust axis to truncate outliers
    plt.savefig(f'out/ggraph2/{k}clique/connectnum-OGnum_type_dist1-2.png')
    plt.close()
    ...
    plt.title('Distribution of connected components across number of OGs')
    plt.legend()
    plt.savefig(f'out/ggraph2/{k}clique//connectnum-OGnum_type_dist2-1.png')
    plt.xlim((-1, 17))  # Adjust axis to truncate outliers
    plt.savefig(f'out/ggraph2/{k}clique/connectnum-OGnum_type_dist2-2.png')
    plt.close()
    ...
    plt.savefig(f'out/ggraph2/{k}clique/type_pie.png')
    plt.close()
...
# Load OG4s
OG4s = {}
with open('../subcluster4_ggraph/out/ggraph2/gclusters.txt') as file:
    for line in file:
...
for CCid, CC in CCs:
    ...
    try:
        ...
        if CCid in ['0873', '08b2', '08b3', '08b4', '08b5']:
            raise CliqueError
        signal.alarm(30)
