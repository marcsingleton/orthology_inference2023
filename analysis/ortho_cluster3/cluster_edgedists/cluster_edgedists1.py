...
if not os.path.exists('out/pgraph1/'):
    os.makedirs('out/pgraph1/')  # Recursive folder creation

bins = linspace(0, hits['bitscore'].max(), 300, endpoint=True)

# 1 Query plots
if not os.path.exists('out/pgraph1/qhists/'):
    os.mkdir('out/pgraph1/qhists/')
...
plt.savefig('out/pgraph1/bar_bitscoremean-qspecies.png')
plt.close()
...
for qspid, group in g:
    ...
    plt.savefig(f'out/pgraph1/qhists/{qspid}.png')
    plt.close()

# 2 Subject plots
if not os.path.exists('out/pgraph1/shists/'):
    os.mkdir('out/pgraph1/shists/')
...
plt.subplots_adjust(bottom=0.15)
plt.savefig('out/pgraph1/bar_bitscoremean-sspecies.png')
plt.close()
...
for sspid, group in g:
    ...
    plt.xlabel('Bitscore')
    plt.ylabel('Number of hits')
    plt.savefig(f'out/pgraph1/shists/{sspid}.png')
    plt.close()

# 3 Query-subject plots
if not os.path.exists('out/pgraph1/qshists/'):
    os.mkdir('out/pgraph1/qshists/')
...
plt.savefig('out/pgraph1/hist_qsnum-bitscoremean.png')
plt.close()
...
for (qspid, sspid), group in g:
    ...
    plt.xlabel('Bitscore')
    plt.ylabel('Number of hits')
    plt.savefig(f'out/pgraph1/qshists/{qspid}-{sspid}.png')
    plt.close()
