...
for i, (component_id, component) in enumerate(component_records[:num_components]):  # Largest components only
    ...
    for k, OGs in zip(ks, OGs_k):
        fig = plt.figure(figsize=figsize)
        ax = fig.add_axes(axes_rect)
        nx.draw_networkx_edges(nx_graph, positions, ax=ax, alpha=edge_alpha1, width=edge_width1)
        nx.draw_networkx_nodes(nx_graph.nodes, positions, ax=ax, node_size=node_size, linewidths=0, node_color=get_node_colors(nx_graph, OGs.get(component_id, [])))
